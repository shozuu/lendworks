		...
		<SheetContent side="left" class="flex flex-col h-full max-h-screen gap-2 p-0">
			...
			<div class="h-14 sticky top-0 flex items-center justify-center border-b">
				<Link :href="route('home')">
					<Logo class="h-11 w-11" :fill="theme === 'dark' ? '#FFFFFF' : '#09090B'" />
				</Link>
			</div>
			...
			<nav class="flex-1 px-4 overflow-y-auto text-lg font-medium">
				...
				<NavLink routeName="my-rentals" componentName="MyRentals/Index" size="sm">
					<HandHelping class="w-5 h-5" />
					My Rentals
				</NavLink>

				<NavLink routeName="my-listings" componentName="MyListings/Index" size="sm">
			...
			<!-- sidebar footer -->
			<Link :href="route('listing.create')" class="sticky bottom-0 p-4 border-t">
				<Button size="sm" class="w-full" variant="outline"> Create Listing </Button>
			</Link>
		</SheetContent>
	</Sheet>
</template>
